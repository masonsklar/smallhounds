...
function create() {
	...
	var barKey = this.add.sprite(408, -196, 'menubaratlas', 'key').setOrigin(0).setInteractive().setAlpha(0);

	windowMask = this.add.sprite(-178, 602, 'windowmask').setOrigin(0);
	windowMask.add = false;
	var rightWindow = this.add.sprite(2146, 546, 'windowatlas', 'rightwindow').setOrigin(0);
	...
	textBox.ready = false;
	textBox.pos = 1;
	var textBoxText = this.add.text(240, 994, ['Hey! Welcome home! This is a small demo for', 'a game I made in Phaser 3. It\'s a promotional', 'tie-in for a music project by Austin Heller', 'called smallhounds!'], {
		fontFamily: 'Arial',
		fontSize: 54,
		color: '#225a89'
	}).setOrigin(0);
	...
	downPrompt.alpha = 1;
	// !left room
	var sizePoster = this.add.sprite(1734, 773, 'sizeposteratlas', 'sizeposter0').setOrigin(0).setInteractive(sizePosterPolygon, Phaser.Geom.Polygon.Contains);
	...
	var helps= {
		l:[
			['I\'m really not sure how I got locked inside!','How embarassing.'],
		],
		m:[
			['Things are always falling into my couch.', 'I try to play memory games to keep my mind sharp.'],
		],
		r:[
			['I love to read and I', 'love to sing :)'],
		],
	};
	...
	var controlStrip = this.add.sprite(2530, 600, 'shooteratlas', 'cs00').setOrigin(0).setInteractive();
	var controlStripTop = this.add.sprite(2530, 600).setOrigin(0).setInteractive(CSTopPoly, Phaser.Geom.Polygon.Contains);
	var controlStripMid = this.add.sprite(2530, 600).setOrigin(0).setInteractive(CSMidPoly, Phaser.Geom.Polygon.Contains);
	var controlStripBot = this.add.sprite(2530, 600).setOrigin(0).setInteractive(CSBotPoly, Phaser.Geom.Polygon.Contains);
	controlStripTop.name = "controlstriptop";
	controlStripMid.name = "controlstripmid";
	controlStripBot.name = "controlstripbot";
	var shooterBg = this.add.sprite(2776, 536, 'flyatlas', '0').setOrigin(0);
	var pane1 = this.add.sprite(2776, 536, 'obstacleatlas', '0').setOrigin(0);
	var pane2 = this.add.sprite(2912, 600, 'obstacleatlas', '0').setOrigin(0);
	var pane3 = this.add.sprite(3050, 664, 'obstacleatlas', Math.floor(Math.random() * 12) + 1).setOrigin(0);
	var p1 = this.add.sprite(2776, 536, 'shooteratlas', '02').setOrigin(0);
	...
	var booksContainer = this.add.container(-880, 860, [book5, book6, book7, book8, book9, book10, book11, topShelf, book1, book2, book3, book4, shelfPlant]).setScale(0.85);
	...
	var widePlant = this.add.sprite(1742, 2060, 'wideplant').setOrigin(0.5,1);
	...
	var sizePosterSounds = [sizePosterSound1, sizePosterSound2, sizePosterSound3, sizePosterSound4, sizePosterSound5];
	...
	textBox.on('pointerdown', function() {
		if (textBox.ready == true && textBox.pos == 1) {
			...
			textBox.pos = 2;
			textBoxText.setText(['I hope you like it!', 'If you have any questions/comments/concerns,','please let me know at me@masonsklar.com.','Happy clicking! : - )']);
			setTimeout(function() {
				textBox.ready = true;
				downPromptAnim.resume();
			}, 1250);
	...
	textBox.on('pointerdown', function() {
		if (textBox.ready == true && textBox.pos == 2) {
			downPromptAnim.pause();
	...
	var uiContainer = this.add.container(1600, 566, [menuBar, barTitle, barKey]);
